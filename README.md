...
with everyone's bank accounts, allowing customers to purchaseMenu products from their computers for their convenience.
...
with a list of all items in the vending machine with its quantity remaining:
    - Each vending machine product has a slot identifier and a purchaseMenu price.
...
process menu:
    ...
    ```
7. The purchaseMenu process flow is as follows:
    ...
    4. After completing their purchaseMenu, the user returns to the "Main" menu to
    continue using the vending machine.
8. The vending machine logs all transactions to prevent theft from the vending machine.
   - Each purchaseMenu must generate a line in a file called `Log.txt`.
...
| Price         | The purchaseMenu price for the product.                                |
